...
test("requireName finds wrapped require with arbitrary wrapper name", () => {
  const source = `let a=1,electronAlias=myCustomWrapper(require(\`electron\`)),c=3`;
  assert.strictEqual(requireName(source, "electron"), "electronAlias");
});

test("requireName finds wrapped require with different module", () => {
  const source = `const fsAlias=wrapperFunc(require("node:fs"))`;
  assert.strictEqual(requireName(source, "node:fs"), "fsAlias");
});
...
test("inferModuleAlias delegates to requireName for wrapped require", () => {
  const source = `let electronAlias=someWrapper(require("electron"))`;
  assert.strictEqual(inferModuleAlias(source, "electron"), "electronAlias");
});
