...
const CONVERSATION_RUNTIME_VERSION = "agent-workspace-conversation-v12";
...
const METHOD="linux-agent-workspace",LAYOUT_KEY="codex-linux-agent-workspace-layout-v1",MIN_WIDTH=280,MIN_HEIGHT=220;let seq=0,pending=new Map,state={panel:null,viewport:null,media:null,head:null,title:null,meta:null,details:null,detailsButton:null,stop:null,revoke:null,refresh:null,resize:null,error:null,activeId:null,visible:false,detailsOpen:false,pollTimer:null,viewTimer:null,viewObserver:null,busy:false,lastImage:"",layout:null,drag:null};
...
function uniqueLabels(labels){let seen=new Set(),out=[];for(let label of labels){let value=shortText(label);if(!value||seen.has(value))continue;seen.add(value);out.push(value)}return out}
function appDetailText(apps){let labels=uniqueLabels(runningApps(apps).map(appLabel));if(labels.length===0)return"No running apps";let visible=labels.slice(0,4).join(", "),extra=labels.length>4?" +"+(labels.length-4)+" more":"";return visible+extra}
function windowDetailText(observe){let active=observe?.json?.active_window,title=shortText(active?.title);if(!title)return"No active window";return title}
function viewerDetailsText(status,apps,observe){let lines=["Active window: "+windowDetailText(observe),"Running apps: "+appDetailText(apps)];let policy=policySummary(status);policy&&lines.push("Policy: "+policy);status?.display&&lines.push("Hidden display: "+status.display);return lines.join("\n")}
...
function render(workspace,observe){if(inSettingsView()){hide();return}ensureUi();let status=observe?.json?.status||workspace?.status||workspace?.manifest||{},apps=Array.isArray(status.apps)?status.apps:workspace?.status?.apps||[],id=workspaceId(workspace),meta=statusText(status,apps)||"Workspace active",detail=statusDetailText(status,apps)||id;state.visible=true;state.activeId=id;if(state.panel)state.panel.hidden=false;if(state.title)state.title.textContent=workspaceLabel(workspace);if(state.meta){state.meta.textContent=meta;state.meta.title=detail}if(state.details)state.details.textContent=viewerDetailsText(status,apps,observe);updateDetailsVisibility();setImage(observe?.json?.screenshot?.data_url);showError("")}
...
function mcpConfigView(config){
  ...
  return h("div",{className:"rounded-md border border-token-border-default bg-token-bg-primary p-3 text-sm"},
    ...
    !locked?h("div",{className:"mt-2 text-xs text-token-text-secondary"},"No MCP ceiling is active. Codex session permissions apply after hidden-workspace approval."):null,
...
function AgentWorkspacesSettings(){
  ...
  var refresh=React.useCallback(async function(){
    ...
    setMcpConfig(nextMcpConfig||{configured:false,restricted:false,message:"No MCP permission ceiling detected; Codex session permissions apply"});
