...
const CONVERSATION_RUNTIME_VERSION = "agent-workspace-conversation-v9";
...
function workspaceViewerThemeOverrides(){return"/* codex-linux-agent-workspace-theme-v9 */"+
...
function installInteractions(){let style=document.getElementById("codex-linux-agent-workspace-style");if(style&&!style.textContent.includes("codex-linux-agent-workspace-theme-v9"))style.textContent+=workspaceViewerThemeOverrides();state.head?.addEventListener("pointerdown",event=>beginInteraction("move",event));state.resize?.addEventListener("pointerdown",event=>beginInteraction("resize",event));window.addEventListener("resize",()=>{if(state.layout)setPanelLayout(state.layout,!0)})}
