...
function agentWorkspaceBridgeWithWorkspaceStartSource(args) {
  const viewerSpawnSource = `if(__codexActionName===\`workspaceOpenViewer\`){try{let e=${args.childProcessVar}.spawn(__codexCommand,__codexArgs,{detached:!0,stdio:\`ignore\`});e?.unref?.();return{ok:!0,action:__codexActionName,command:__codexCommand,args:__codexArgs,json:{ok:!0,id:__codexString(__codexWorkspaceId)||\`default\`,pid:e?.pid??null,always_on_top:!!__codexAlwaysOnTop}}}catch(e){return{ok:!1,action:__codexActionName,command:__codexCommand,args:__codexArgs,message:e instanceof Error?e.message:String(e)}}}let __codexParse=`;
  const viewerSpawnSafeSource = `if(__codexActionName===\`workspaceOpenViewer\`){return await new Promise(e=>{let t=!1,n=null,r=a=>{if(t)return;t=!0,n&&clearTimeout(n),e(a)},m=a=>a instanceof Error?a.message:String(a);try{let a=${args.childProcessVar}.spawn(__codexCommand,__codexArgs,{detached:!0,stdio:\`ignore\`}),p={ok:!0,action:__codexActionName,command:__codexCommand,args:__codexArgs,json:{ok:!0,id:__codexString(__codexWorkspaceId)||\`default\`,pid:a?.pid??null,always_on_top:!!__codexAlwaysOnTop}};a?.once?.(\`error\`,e=>r({ok:!1,action:__codexActionName,command:__codexCommand,args:__codexArgs,message:m(e)}));a?.once?.(\`spawn\`,()=>{a?.unref?.();r(p)});n=setTimeout(()=>{a?.unref?.();r(p)},25)}catch(a){r({ok:!1,action:__codexActionName,command:__codexCommand,args:__codexArgs,message:m(a)})}})}let __codexParse=`;
